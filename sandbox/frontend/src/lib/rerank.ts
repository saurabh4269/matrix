...
export function rerank(
  ...
  const composites = candidates.map(c => {
    // Defensive against backends that don't ship breakdown (older builds):
    // fall back to keeping the original score unchanged.
    const b = c.breakdown ?? { must_have: 0, substance: 0, retrieval: 0, location: 0 }
